...
class BacktestResults:
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "max_drawdown_duration_bars": m.max_drawdown_duration,
            "win_rate_pct": round(m.win_rate * 100, 2),
            "profit_factor": round(m.profit_factor, 3),
            "expectancy": round(m.expectancy, 2),
            "n_trades": m.n_trades,
            "var_95_pct": round(m.value_at_risk_95 * 100, 3),
            "cvar_95_pct": round(m.conditional_var_95 * 100, 3),
...
class BacktestEngine:
    ...
    def run(
        ...
        Parameters
        ----------
        signals_df        : DataFrame indexed by date; columns = assets; values = target
                            weight in [-1, 1] (0 = flat).
        prices_df         : DataFrame indexed by date; columns = assets; OHLCV columns
                            expected as MultiIndex (asset, field) or simple close prices.
    ...
    def walk_forward_validation(
        ...
        for split in range(n_splits):
            ...
            # In this simplified version signals = uniform zero (no model)
            # In practice the caller would pass trained models per split.
    ...
    def generate_report(self, results: BacktestResults) -> dict:
        """Return full performance report as a plain dict."""
        report = results.summary()
        report["avg_trade_pnl"] = round(results.metrics.avg_trade_pnl, 2)
        report["std_trade_pnl"] = round(results.metrics.std_trade_pnl, 2)
        report["beta"] = round(results.metrics.beta, 3)
        report["alpha_annualized"] = round(results.metrics.alpha, 4)
        return report
    ...
    def _build_trade_log(rows: List[dict], prices: pd.DataFrame) -> pd.DataFrame:
        ...
        if not rows:
            return pd.DataFrame(
                columns=[
                    "date", "symbol", "shares_delta", "execution_price",
                    "notional", "transaction_cost", "slippage_cost", "pnl",
